...
@endif
                              ...
                              <form method="POST" id="login-form" action="{{ route('front.log.forgetpassword_rest',$token) }}">
                        @csrf
                        <input type="hidden" name="token" value="{{$token}}">
                                <label class ="@error('email') text-danger @enderror" >{{ __('E-Mail Address') }}</label>
                                <input type="text" name="email" class="form-control @error('email') is-invalid @enderror" placeholder="{{ __('E-Mail Address') }}" value="{{ old('email') ?? $email }}" required autocomplete="email" autofocus>
                                 @error('email')
                                       <div class="error invalid-feedback" >{{ $message }}</div>
                                @enderror
                                <label class ="@error('opt') text-danger @enderror">{{ __('opt') }}</label>
                                <input type="text" name="opt" class="form-control @error('opt') is-invalid @enderror" placeholder="{{ __('opt') }}">
                                 @error('opt')
                                   <div class="error invalid-feedback" >{{ $message }}</div>
                                @enderror
                                 <label id="label-password" class="@error('password') text-danger @enderror mt-1 ">@lang('label.password'):<span class="text-danger">*</span></label>
                                <input type="password" id="parent-password"  class="form-control @error('password') is-invalid @enderror" name="password" placeholder="@lang('label.password')">
                                     @error('password')
                                   <div class="error invalid-feedback" >{{ $message }}</div>
                                @enderror 
