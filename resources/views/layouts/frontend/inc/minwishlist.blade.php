...
@endphp
<input type="hidden" id="wishlist-count-item" value="{{$count_wishlist}}">

            @forelse($wishlist as $wish)
            
                   <div class="media d-flex align-items-center">
                        
                            <img src="{{asset('public/storage'.$wish->image)}}" class="mini-thumb">
                            <div class="media-body">
                            <h5 class="mt-0"><a href="{{route('shoping.producd.detils',$wish->object_id)}}">{{$wish->object_name}}</a></h5>
                                    <a href="javascript:void(0)" id="product-list-cart-{{$wish->object_p_id}}" data-deal-id="#deal-list-cart-{{$wish->object_p_id}}" @if(in_array($wish->object_id,$cart_var)) class="wish-min wish_active text-white" @else class="wish-min" @endif data-url="{{route('shoping.cart.add',$wish->object_p_id)}}" onclick="showview(this)" data-viewport="mini-shoping-card"  data-shoping-cart="#shoping-count-item" data-shoping-cart-total="#shoping-total" data-shoping-class="wish_active" data-alert-type="success" data-alert-text="@lang('message.product_added_to_cart_successfully')" data-alert-timer="2000">
              ...
              viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                ...
                <path d="M443.188,442.208L415.956,142.56c-0.768-8.256-7.68-14.56-15.968-14.56h-48V96c0-25.728-9.952-49.888-28.032-67.968
                  ...
                  C412.116,454.176,409.204,462.88,403.06,469.6z"/>
                            ...
                            <span class="close"><a href="javascript:void(0)" data-shoping-cart-total="#wishlist-count-item"><i class="far fa-times-circle"></i></a></span>
                          </div>
                        
                                              
    @empty
    <p>@lang('label.no_wishlist_found')</p>
@endforelse
